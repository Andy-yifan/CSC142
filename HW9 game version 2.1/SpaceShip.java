...
public class SpaceShip extends MovingObject {
	...
	private static Sound explosion = new Sound("shoot.wav");
	 ...
	 */
	public void shoot(ArrayList<Alien> aliens,ArrayList<Alien2> aliens2) {
		explosion.play();
		...
		if(aliens2 != null && aliens2.size() != 0){
			...
			Rectangle box = a2.getBoundingBox();
			if (center.x >= box.getX()
					&& center.x <= box.getX() + box.getWidth()) {
				a2.isShot();
				score += 100;
	...
	protected void draw() {
		...
		this.shapes[5] = new TextShape("Score: " + String.valueOf(score),10,10,Color.white);
