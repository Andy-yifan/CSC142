...
public class Alien extends MovingObject {
	...
	public void move() {
		...
		int reset = 0;
		// update the location
		center.x += rnd.nextInt(10) ;
		if(center.x % 2 == 0){
			center.y += rnd.nextInt(10) ;
		}else if (center.y % 2 == 1){
			center.x -= rnd.nextInt(10) ;
		}
		if(center.x >window.getWindowWidth()){
			center.x =0 ;
			reset= window.getWindowWidth();
		}
		// if pass the bottom of the window, bring it back to the top
		
		int reset2 = 0;
		if (center.y > window.getWindowHeight()){
			center.y = 0;
			reset2 = window.getWindowHeight();
		}
		// to move the alien
		erase();
		draw();
